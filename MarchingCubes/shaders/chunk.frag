...
void main()
{
    ...
    vec3 grassColor = (vec3(37, 135, 7) / 255.0) * (1.0 + simplex3d(frag_pos * 10) * 0.1 + simplex3d(frag_pos * 0.05) * 0.4);
   	vec3 dirtColor = vec3(70, 46, 26) / 255.0;
    
    float steepness = dot(normal, vec3(0, 1, 0));
   	vec3 blendedColor = blendColors(dirtColor, grassColor, 0.7, 0.75, steepness);
	
    gColor.rgba = vec4(blendedColor, 1.0);
